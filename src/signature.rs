...
/// 按输出编码格式编码签名字节为字符串。
fn encode_output(sig: &[u8], output: &str) -> Result<String, PhpException> {
...
/// 按输出编码格式解码签名串为字节。
fn decode_output(sig: &str, output: &str) -> Result<Vec<u8>, PhpException> {
    match output.to_lowercase().as_str() {
        "hex" => hex::decode(sig).map_err(|e| xhsm_exception(format!("签名 hex 解码失败: {}", e))),
